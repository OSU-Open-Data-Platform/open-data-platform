...
class FacetsFilter extends FilterPluginBase {
  ...
  protected function defineOptions() {
    ...
    unset($options["expose"]["contains"]["remember"]);
    return $options;
  }
   ...
   * Can be called recursively to build a hierarchy.
   *
  ...
  private function getActiveFacetValues() {
    // @todo clean up to use $this->request->query instead, see QueryString.php
    if (!isset($_GET[$this->options["expose"]["identifier"]])) {
      return [];
    }
    $enabled = $_GET[$this->options["expose"]["identifier"]];
    if ($enabled == 'All') {
      return [];
    }
